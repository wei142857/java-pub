...
;(function($) {
    ...
    $.omWidget("om.omTree", {
        ...
        /**
         *  filter_config 中 .checkbox_full改为.checkbox_full,.checkbox_part  将所有被勾选的节点都获取到  by xuml
         * 获取所有被勾选的节点
         */
        getCheckedNew: function(checked) {
            var self = this,
                nodes = [];
        ...
        /**
         * 第二种数据结构 修改指定节点后保留子节点
         */
        modifyNew: function(target, newNode, pNode) {
        	if(target&&newNode){
        		var self = this, nextNode = $("#" + target.nid).next(), bNode;
                pNode = pNode || this.findByNId(self.element.data("nodes")["pid" + target.nid]);
                if(nextNode.is("ul") || nextNode.is("li"))
                    bNode = self.findByNId(nextNode.attr("id"));
                var chidrens = self.element.data("nodes")[target.nid].children;
                
                self.remove(target, pNode);
                newNode.children = chidrens;
                self.insert(newNode, pNode, bNode);	
        	}
        },
