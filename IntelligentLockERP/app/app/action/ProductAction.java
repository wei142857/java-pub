...
public class ProductAction extends BaseAction{
	...
	public static Result getProductList(){
		...
		if(StringUtil.isNull(token)||StringUtil.isNull(phone)){
	...
	public static Result exchangeProduct(){
		...
		if(StringUtil.isNull(token)||StringUtil.isNull(phone)||StringUtil.isNull(smscode)||StringUtil.isNull(pid)){
			reJson.setParamsError();
			String reContent = JsonUtil.parseObj(reJson);
			appLogger.debug(reContent);
			return ok(reContent);
		...
		if(appUser==null){
			...
			Integer result = ProductService.payBountyExchangeProduct(appUser.getIdd(), phone, "APP", product.getPrice(), npid, appUser.getIp());
			if(result == 1){
				reJson.setSuccess();
				String reContent = JsonUtil.parseObj(reJson);
				appLogger.debug(reContent);
	...
	public static Result getExchangeList(){
		...
		if(appUser==null){
			...
			if(exchangeList != null && exchangeList.size()>0){
				...
				List<SmartExchanges> allExchanges = ProductService.selectExchangesRecordByUserid(appUser.getIdd());
				//play.Logger.info("exchangeList"+exchangeList+","+data+","+npagesize);
				if(allExchanges.size()%npagesize==0){
					data.setTotalpages(allExchanges.size()/npagesize);
					data.setPageno(npageno);
				}else{
					data.setTotalpages((allExchanges.size()/npagesize)+1);
					data.setPageno(npageno);
				}
